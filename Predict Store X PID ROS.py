...
def analyze_prediction_distribution(df, store_code):
    ...
    store_df['Ratio'] = store_df['Predicted Store X PID ROS'] / store_df['Store X PID ROS'].replace(0, np.nan)
    store_df['Ratio'] = store_df['Ratio'].fillna(0)
    store_df['Ratio'] = round(store_df['Ratio'] / 0.1).astype(int) * 0.1
    store_df['Store X PID ROS'] = round(store_df['Store X PID ROS'] / 0.1).astype(int) * 0.1
